<?php
	...
	if($proses=="cetak"){
		...
		switch($pilihan){
			default:
				$que0 	= "SELECT a.periode,a.gol_kode,SUBSTR(a.gol_ket,1,14) AS gol_ket,COUNT(a.rek_nomor) AS lembar,SUM(a.pemakaian) AS pemakaian,SUM(a.rek_uangair) AS rek_uangair,SUM(a.beban_tetap) AS beban_tetap,SUM(a.rek_angsuran) AS rek_angsuran,SUM(a.rek_denda) AS rek_denda,SUM(a.rek_materai) AS rek_materai,SUM(a.byr_total) AS byr_total FROM v_lpp a WHERE a.kar_id='".$kar_id."' AND DATE(a.byr_tgl)=STR_TO_DATE('$dibayar','%Y-%m-%d') AND a.byr_sts>=1 GROUP BY a.periode,a.gol_kode ORDER BY a.periode DESC,a.gol_kode ASC";
...
?>
	<tr class="<?php echo $klas; ?>">
		<td class="right prn_cell prn_left" style="color: <?=$warna?>"><?php echo $gol_ket;	 			?></td>
