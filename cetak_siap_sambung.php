<?php
	...
	if($gol_kode=='SL'){
		$que0 	= "SELECT a.* FROM v_siap_sambung a WHERE a.kp_kode='".$kopel[0]."' ORDER BY a.pel_no";
	}
	else if($gol_kode=='KW'){
		$que0 	= "SELECT a.* FROM v_siap_sambung a WHERE a.kp_kode='".$kopel[0]."' ORDER BY a.dkd_kd";
	}
	else{
		$que0 	= "SELECT a.* FROM v_siap_sambung a WHERE a.kp_kode='".$kopel[0]."' ORDER BY a.pel_no";
	}
