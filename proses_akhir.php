	...
	if(isset($proses)){
		...
		try {
			...
			if($st>0){
				...
				$mess = "Loket pembayaran telah ditutup. Klik <u onclick=\"buka('080303')\">di sini</u> untuk membuka menu Cetak LPP Rinci";
				$klas = "success";
			}
		}
		catch (PDOException $err){
...
<div id="bukaLoket">
	<table>
		<tr valign="top"> 
			<td colspan="2">
				<p style="padding:6px; font-size:14px;">
					Anda login sebagai <b><?=$_SESSION['Name_c']?></b><br/>
					Akses dari IP <?=_HOST?>
				</p>
				<hr/>
			</td>
		...
		<tr valign="top"> 
			...
			<td width="70%">:
				<input readonly type="text" name="tanggal" size="15" value="<?=date('d-m-Y')?>" />
			</td>
		</tr>
		<?php if($stsLoket){ ?>
		<tr>
