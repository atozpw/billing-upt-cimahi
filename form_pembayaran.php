...
<?php
	switch($proses){
		case "bayar":
			include _PROC;
			$cetakin	= "<input type=\"button\" class=\"cetak\" value=\"Cetak Bukti Bayar\" onclick=\"nonghol('cetak')\" />";
...
<input type="hidden" class="cetak" 	name="targetUrl" value="cetak_resi.php"/>
<input type="hidden" class="cetak" 	name="byr_no" 	 value="<?php echo $byr_no; ?>"/>
<?php
			echo "<div class=\"$klas cetak\">$mess</div>";
			...
			try{
				...
				if(!$res0 	= mysql_query($que0,$link)){
					...
					while($row0 = mysql_fetch_array($res0)){
						...
						$data[$i]['rek_beban']	= $row0['rek_adm']+$row0['rek_meter'];
						//$grandTotal[$i]			= $row0['rek_total']+$row0['rek_denda']+$row0['rek_materai'];
						$grandTotal[$i]			= $row0['rek_total']+$row0['rek_denda'];
						$pel_no					= $row0['pel_no'];
...
?>
	<tr class="<?=$class_nya?>" >
		...
		<td>
			...
			<input type="hidden" class="noRek" name="rek_adm[<?php echo $i;?>]"		value="<?php echo $rek_adm;		?>"/>
			<input type="hidden" class="noRek" name="rek_meter[<?php echo $i;?>]"		value="<?php echo $rek_meter;		?>"/>
			...
			<input id="form-<?=($i+1)?>" type="checkbox" class="pilih" onclick="bayarRek('<?=$i?>')"/>
		</td>
	</tr>
<?php
				}
...
?>					   				   
	<tr class="table_head"> 
		...
		<td align="center">
			<input id="form-<?=$i?>" type="button" value="Hitung" onClick="nonghol('kalkulator')"/> 
		</td>				
	</tr>
</table>
<?php
