<?php
	...
	if($gol_kode=='SL'){
		$que0 	= "SELECT a.* FROM v_pembukaan a WHERE a.bulan=$rek_bln AND a.tahun=$rek_thn AND a.kp_kode='".$kopel[0]."' ORDER BY a.pel_nama";
	}
	else if($gol_kode=='KW'){
		$que0 	= "SELECT a.* FROM v_pembukaan a WHERE a.bulan=$rek_bln AND a.tahun=$rek_thn AND a.kp_kode='".$kopel[0]."' ORDER BY a.dkd_kd";
	}
	else{
