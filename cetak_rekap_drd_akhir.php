<?php
	...
	switch($gol_kode){
		case "KW":
			$que0 	= "SELECT CONCAT(b.dkd_kd,' [',b.dkd_jalan,']') AS rek_gol,COUNT(a.rek_nomor) AS rek_lembar,SUM(ABS(a.rek_stankini-a.rek_stanlalu)) AS rek_pakai,SUM(a.rek_uangair) AS rek_uangair,SUM(a.rek_adm+a.rek_meter) AS rek_meter,SUM(a.rek_total) AS rek_total FROM tm_rekening a JOIN tr_dkd b ON(b.dkd_kd=a.dkd_kd) JOIN tm_pelanggan c ON(c.pel_no=a.pel_no) WHERE a.rek_sts=1 AND a.rek_bln=".$rek_bln." AND a.rek_thn=".$rek_thn." ".$filter." GROUP BY a.dkd_kd"; 
			$kate	= "Wilayah";
			break;
		default :
			$que0 	= "SELECT CONCAT(b.gol_kode,' [',b.gol_ket,']') AS rek_gol,COUNT(a.rek_nomor) AS rek_lembar,SUM(ABS(a.rek_stankini-a.rek_stanlalu)) AS rek_pakai,SUM(a.rek_uangair) AS rek_uangair,SUM(a.rek_adm+a.rek_meter) AS rek_meter,SUM(a.rek_total) AS rek_total FROM tm_rekening a JOIN tr_gol b ON(b.gol_kode=a.rek_gol) JOIN tm_pelanggan c ON(c.pel_no=a.pel_no) WHERE a.rek_sts=1 AND a.rek_bln=".$rek_bln." AND a.rek_thn=".$rek_thn." ".$filter." GROUP BY a.rek_gol"; 
			$kate	= "Golongan";
	}
